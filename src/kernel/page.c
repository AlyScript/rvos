...
__attribute__((section(".text.sys"))) void page_init() {
  ...
  uint64_t entry = create_pte(
      gigapage_base, 0xF); /* Flags: XWRV. TODO: come back and change clear W for instructions and X for data. */
  __root_pte[kernel_addr >> 30] =
      entry; /* MMU will take the upper 9 bits of the 39 bit address and use it as an index into the root table */
