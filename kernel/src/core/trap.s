...
mhandler
    ...
    subi sp, sp, 12                       ; Push working registers and ra
    sw ra, 8[sp]                          
    sw t0, 4[sp]                          
    sw t1, [sp]   
...
; ------------------------------------------ Interrupt Handling ------------------------------------------
    ...
    lw t1, [t1]                           ; Load the address of the handler
...
interrupt_table
    DEFW handle_interrupt_0             ; 0 - User Software Interrupt
    DEFW handle_interrupt_1             ; 1 - Supervisor Software Interrupt
    DEFW handle_interrupt_2             ; 2 - Reserved
    DEFW handle_interrupt_3             ; 3 - Machine Software Interrupt
    DEFW handle_interrupt_4             ; 4 - User Timer Interrupt
    DEFW handle_interrupt_5             ; 5 - Supervisor Timer Interrupt
    DEFW handle_interrupt_6             ; 6 - Reserved
    DEFW handle_interrupt_7             ; 7 - Machine Timer Interrupt
    DEFW handle_interrupt_8             ; 8 - User External Interrupt
    DEFW handle_interrupt_9             ; 9 - Supervisor External Interrupt
    DEFW handle_interrupt_10            ; 10 - Reserved
    DEFW handle_machine_external        ; 11 - Machine External Interrupt
...
handle_machine_external:
    ...
    subi sp, sp, 36                         ; Push working registers and ra to be safe
    sw ra, 32[sp]
    sw t0, 28[sp]
    sw t1, 24[sp]
    sw t2, 20[sp]
    ...
    lw t1, [t0]                             ; Read the interrupt status register
...
handle_ecall_umode:   
    ...
    lw t0, [t0]                 ; Load the address of the ecall handler
